export class MgDataLabel extends HTMLElement {
  ...
  private get label() { return this.getAttribute("label") || "STATUS"; }
  ...
  private render() {
    ...
    this.innerHTML = `<span style="font-family:monospace;font-size:10px;display:inline-flex;align-items:center;gap:0;border:1px solid var(--border)">
      <span style="color:${color};padding:1px 4px;border-right:1px solid var(--border);background:var(--bg-secondary)">${this.label}</span>
